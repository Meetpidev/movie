...
function TheaterMovies() {
  ...
  const handleDeleteMovie = async (id, e) => {
    e.stopPropagation();
    const movieToDelete = movies.find(movie => movie._id === id);
    if (movieToDelete.theater.admin === localStorage.getItem("adminId")) {
      try {
  ...
  const handleEditClick = (movie, e) => {
    e.stopPropagation();
    if (movie.theater.admin === localStorage.getItem("adminId")) {  // Assuming you store adminId in localStorage
      setIsEditing(true);
      setEditMovie(movie);
    } else {
